...
class ProductPage {
    ...
    addProducts = async (table) => {
        ...
        for (const element of tableRow) {
            ...
            const btnSubCategory = $(`//div[@class='subcategory-image']/a[contains(@title,'${element.subCategory}')]`);
    ...
    buyProduct = async () => {
        ...
        await expect(
            ...
            staticData.address.firstName +
            " " +
            staticData.address.lastName
        );

        await expect(await this.txt_DeliveryAddressCompany.getText()).toEqual(
            staticData.address.company
        );

        await expect(await this.txt_DeliveryAdd_Add1_Add2.getText()).toEqual(
            staticData.address.address1 +
            " " +
            staticData.address.address2
        );
